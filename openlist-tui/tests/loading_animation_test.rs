...
fn test_loading_start() {
    ...
    assert!(!app.pending_task.is_loading());
    assert!(app.loading_message.is_none());
    assert!(app.loading_progress.is_none());

    // Start loading
    app.start_loading("Loading files...".to_string());

    // Should be in loading state
    assert!(app.loading_message.is_some());
    assert_eq!(app.loading_message, Some("Loading files...".to_string()));
...
fn test_loading_stop() {
    ...
    // Start loading
    app.start_loading("Loading...".to_string());
    assert!(app.loading_message.is_some());
...
fn test_progress_update() {
    ...
    assert!(app.loading_progress.is_some());
    assert_eq!(app.loading_progress, Some((5, 10)));

    // Update progress again
    app.update_progress(8, 10);
    assert_eq!(app.loading_progress, Some((8, 10)));
}
...
fn test_spinner_frame_rotation() {
    ...
    for i in 1..=10 {
        app.advance_spinner();
        if i < 10 {
            assert_eq!(app.loading_spinner_frame, i);
        } else {
...
fn test_spinner_characters() {
    ...
    for expected_char in expected_chars.iter() {
        assert_eq!(app.get_spinner_char(), *expected_char);
        app.advance_spinner();
    }
...
fn test_app_pending_task_integration() {
    ...
    assert!(app.pending_task.is_loading());
    assert_eq!(app.pending_task.get_message(), Some("Connecting..."));

    // Advance spinner through pending task
    app.pending_task.advance_spinner();
    assert_eq!(app.pending_task.get_spinner_char(), '⠙');
    ...
    assert_eq!(app.pending_task.get_progress(), Some((10, 20)));
    assert_eq!(app.pending_task.get_message(), Some("Renaming..."));
}
...
fn test_loading_with_progress_percentage() {
    ...
    app.start_loading("Batch processing...".to_string());
    ...
    for (completed, total, _expected_percent) in test_cases {
        app.update_progress(completed, total);
        assert_eq!(app.loading_progress, Some((completed, total)));
    }
}
...
fn test_spinner_frame_bounds() {
    ...
    for _ in 0..100 {
        app.advance_spinner();
        assert!(app.loading_spinner_frame < 10);
    }

    // get_spinner_char should never panic
    for _ in 0..100 {
        let _char = app.get_spinner_char();
        app.advance_spinner();
    }
}
...
fn test_loading_state_transitions() {
    ...
    // Start loading
    app.start_loading("Step 1".to_string());
    assert!(app.loading_message.is_some());
    assert_eq!(app.loading_message, Some("Step 1".to_string()));

    // Update to different message (simulating new step)
    app.stop_loading();
    app.start_loading("Step 2".to_string());
    assert_eq!(app.loading_message, Some("Step 2".to_string()));
    assert_eq!(app.loading_spinner_frame, 0); // Should reset

    // Add progress
    app.update_progress(3, 5);
    assert_eq!(app.loading_progress, Some((3, 5)));

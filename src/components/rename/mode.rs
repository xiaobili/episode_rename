...
pub fn render(frame: &mut Frame, rename: &RenameState, _config: &Config) {
    ...
    let selected_idx = modes.iter().position(|&m| m == rename.mode_selection.selected_mode).unwrap_or(0);
    ...
    let mode_items: Vec<ListItem> = modes.iter().enumerate().skip(start_idx).take(end_idx - start_idx).map(|(_i, mode)| {
        let prefix = if *mode == rename.mode_selection.selected_mode { "> " } else { "  " };
    ...
    let preview_lines: Vec<Line> = rename.mode_selection.preview.iter().map(|line| {
